...
hav_l_back = zeros(625,1);
hav_r_back = zeros(625,1);
for i = 1 :25
    hav_l_front(((i-1)*25+1): i*25) = hav_l(((i-1)*50+1):((i-1)*50+25));
    hav_r_front(((i-1)*25+1): i*25) = hav_r(((i-1)*50+1):((i-1)*50+25));
    hav_l_back(((i-1)*25+1): i*25) = hav_l(((i-1)*50+26):((i-1)*50+50));
    hav_r_back(((i-1)*25+1): i*25) = hav_r(((i-1)*50+26):((i-1)*50+50));
end

ref_l_front = repmat(hav_l (609), [625 1]); % 左耳 reference
ref_r_front = repmat(hav_r (609), [625 1]);
ref_l_back = repmat(hav_l (641), [625 1]);
ref_r_back = repmat(hav_r (641), [625 1]);
...
output_front = [hav_l_front; hav_r_front];
input_back = [ref_l_back, azi_l, ele_back; ref_r_back, azi_r, ele_back];
output_back = [hav_l_back; hav_r_back];
...
hav_train_ori_front = output_front ([trainSet, validSet]);
hav_test_ori_front = output_front (testSet);
...
[hav_train_ori_front, mu_front_out, sigma_front_out] = zscore(hav_train_ori_front);

[train_in_back, mu_back, sigma_back] = zscore(train_in_back);
test_in_back = normalize(test_in_back, mu_back, sigma_back);
[hav_train_ori_back, mu_back_out, sigma_back_out] = zscore(hav_train_ori_back);

%% Training
% front
rand('state',0)
nn_front = nnsetup([3 180 120 180 1]);
...
[nn_front, L_front] = nntrain(nn_front, train_in_front(1:750, :), hav_train_ori_front(1:750), opts_front, train_in_front(751:930, :), hav_train_ori_front(751:930), 500);

% back
rand('state',0)
nn_back = nnsetup([3 50 180 50 1]);
...
[nn_back, L_back] = nntrain(nn_back, train_in_back(1:750, :), hav_train_ori_back(1:750), opts_back, train_in_back(751:930, :), hav_train_ori_back(751:930), 500);
